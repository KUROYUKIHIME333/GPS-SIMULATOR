...
class TransfertManager {
    private transfers: { [key: string]: Transfert } = {}; // Index signature pour typer l'objet

    private simulationInterval: number = 1000; // Fréquence de mise à jour de la simulation en ms
    private reRouteChance: number = 0.01; // Probabilité de tenter un reroutage dans la fenêtre définie

    // Calcule la durée théorique d'un itinéraire basé sur la distance totale et la vitesse moyenne.
    private _calculateDuration(polyline: Point[], vitesseMoyenne: number): number {
        ...
        const vitesseMoyenne_mps = vitesseMoyenne * 1000 / 3600; // Conversion km/h en m/s
        return totalDistance / vitesseMoyenne_mps; // Durée en secondes
    }

    // Répartit aléatoirement le temps total d'arrêt en plusieurs petites durées.
    ...
    // Définit la fenêtre de temps (en secondes du début du trajet) où un reroutage est permis.
    ...
    // Démarre la simulation d'un transfert de mobile.
    public async startTransfert(mobileId: string, polyline: Point[], vitesseMoyenne: number, probabiliteArret: number, tempsTotalArret: number, reRouteStrategy: ReRouteStrategy): Promise<{ id: string; initialDuration: number }> {
        const id = uuidv4();
    ...
    // La boucle de simulation principale qui met à jour l'état du mobile.
    private async _simulateTransfert(id: string): Promise<void> {
        ...
        if (transfert.isStopped) {
            ...
            if (!transfert.isStopped) {
                ...
                // --- Simulation de changement d'itinéraire ---
                const { min, max } = transfert.reRouteWindow;

                if (!transfert.hasReRouted &&
                    transfert.elapsedTime >= min &&
                    transfert.elapsedTime <= max &&
                    Math.random() < this.reRouteChance &&
                    transfert.reRouteAttempts < 1)
                {
    ...
    // Renvoie l'état actuel d'un transfert.
    public getTransfertStatus(id: string): TransfertStatus | null {
        ...
        const distancePerSecond = transfert.vitesseMoyenne * 1000 / 3600;
        const estimatedRemainingTime = (currentSpeed > 0 && distancePerSecond > 0 && remainingDistanceOnCurrentRoute > 0) ?
            (remainingDistanceOnCurrentRoute / distancePerSecond) :
            Math.max(0, initialTravelTime - totalElapsedTime);
    ...
    // Annule une simulation de transfert en cours.
...
// Exporte une instance unique de TransfertManager
